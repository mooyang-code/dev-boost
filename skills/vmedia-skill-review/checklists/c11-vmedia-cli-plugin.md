# C11 — vmedia-cli 插件作者专项检查

## 标准

本条专供"把 skill 作为 vmedia-cli 插件发布"的作者使用。普通 skill 走 c1–c10 即可；**一旦要发布为插件**，还要额外满足本条。

协议权威定义在 `vmedia-cli/docs/release-pkg-spec.md`，本检查清单为它的 **review 对应物**：开发者在交付前自查、reviewer 在 review 时核对。

## 何时适用

- 作者打算把 skill 打进 vmedia-cli 插件包发布（`vmedia-admin release-pkg`）
- skill 需要附带 CLI 二进制（不是纯文档 skill）
- 发布后会被 vmedia-cli 路由 / 自动安装到终端用户

不适用：纯文档/规范/review skill（如 `vmedia-skill-review` 本身）、团队内部手工分发的 skill。
...
```bash
vmedia-admin release-pkg /path/to/your-plugin-pkg.zip --channel=beta --dry-run
```
...
```text
❌ SKILL.md 写 name: datamind，但二进制叫 vmedia-dm-darwin-arm64
...
❌ 把 4 个平台的 zip 分别发：
   vmedia-admin release-pkg datamind-darwin-arm64.zip  (只含 1 个二进制)
...
| **发布包合规（本条）** | **c11 —— 专供 vmedia-cli 插件发布** |

---

## 参考

- `vmedia-cli/docs/release-pkg-spec.md` —— 完整协议
- `vmedia-cli/skill/admin/SKILL.md` —— Agent 侧发布流程
- `vmedia-cli/skill/admin/references/pkg-spec.md` —— Agent 速查
