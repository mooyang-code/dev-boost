...
真实结果：Agent 直接触发 `datamind-export`，没有读主 `datamind/SKILL.md`，于是执行 `vmedia-dm project list`，再用 `which vmedia-dm` 误判 CLI 未安装。
...
- 引导技能本身**不执行业务**，只做路由：「你是不是想做 X？请去读 vmedia-xxx skill」
