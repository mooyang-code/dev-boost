...
# 2. 把 *.go 文件里的 <YOUR-CLI> 替换为你的 CLI 名（如 moo-foo）
sed -i '' 's/<YOUR-CLI>/moo-foo/g' ./{cli_error,exitcode,term,validate,output}.go
...
示例：
  # 1) UDF 模式 + 试跑（不会真实下单）
  moo-foo clean create --dry-run \
    --project saas_xxx_video_1 --mode udf \
    --udf-name normalize_title --input-file cos://bkt/raw/2026-04-19.csv

  # 2) 真实创建 + 幂等键（重试不重复下单）
  moo-foo clean create --idempotency-key clean-2026-04-19-001 \
    --project saas_xxx_video_1 --mode udf ...

  # 3) 看输出
  moo-foo clean create ... | jq '.task_id'`
```
