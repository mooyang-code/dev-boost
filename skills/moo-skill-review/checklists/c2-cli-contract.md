...
```bash
# Agent 第 1 次调用：source 设置了 $CLI_BIN
unset SKILL_DIR CLI_BIN && source "<SKILL_DIR>/find_cli.sh" && "$CLI_BIN" --help

# Agent 第 2 次调用：是新进程，$CLI_BIN 已经空了
"$CLI_BIN" --token ... --gateway ... project list
# → zsh:1: command not found: --token   （$CLI_BIN 展开为空，--token 被当成命令）
```
...
- [ ] 所有可粘贴命令都以 `unset SKILL_DIR CLI_BIN && source "<SKILL_DIR>/find_cli.sh" && "$CLI_BIN" ...` 开头。
- [ ] 如果存在嵌套子技能，每个 `subskills/**/SKILL.md` 顶部都有“直接加载保护”：说明子技能可能被 Agent 直接触发，并给出完整自举命令模板。
- [ ] 子技能不出现裸 `<cli> ...` / `<cli> ...` 示例；CLI 不要求在 `PATH` 中，所有示例都走 `"$CLI_BIN"`。
- [ ] 文档不引导 `which <cli>` / `command -v <cli>` 判断 CLI 是否安装；正确判断方式是 `source "<SKILL_DIR>/find_cli.sh"` 是否能定位安装目录内二进制。
...
```bash
# ❌ skill 里这样写：
$CLI_BIN --token "$API_TOKEN" --gateway http://gw.test project list
```
...
```bash
# ❌
source "/Users/mooyang/.box/Workspace/find_cli.sh"
# ❌
source "/Users/mooyang/Library/Application Support/Box/engine/skills/user/example-cli/find_cli.sh"
```
...
`<SKILL_DIR>` 应在 SKILL 顶部明确说明：「由你的 Agent 平台告知，通常是 `~/.<your-agent>/skills/example-cli` 或类似目录」。
...
```text
export subskill 被正确触发
Agent 执行：<cli> project list
Agent 执行：which <cli> || echo "<cli> not found in PATH"
Agent 结论：示例 CLI 还没有安装或配置
```
...
# 通用模板（每条命令都按这个格式拼）
unset SKILL_DIR CLI_BIN && source "<SKILL_DIR>/find_cli.sh" \
  && "$CLI_BIN" --token <真实-token-字符串> --gateway <gateway-url> <子命令> [flags]
```
