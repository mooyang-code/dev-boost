...
多个 moo-* skill 触发词有重叠时，大模型每一轮**可能随机选中其中一个**，结果在用户做完一件事后被反复触发到不同 skill，形成 A→B→A→B 的环：
...
### 方案 1：`<MOO-SKILL-REENTRY-GUARD>` 重入防护标签
...
- 标签命名 `<MOO-SKILL-REENTRY-GUARD>` 是 moo 团队自定义命名空间，不与公共 skill 冲突
...
| 写法 | 大模型反应 |
|------|----------|
| ❌ "请 invoke moo-dashboard skill 上传数据" | 直接加载 → 触发回弹 |
| ❌ "完成导出后，调用 moo-dashboard 上传到看板" | 直接加载 → 触发回弹 |
| ✅ "如果用户接下来要把数据传到看板，**等用户明确指令**再切换到 moo-dashboard" | 等用户决定，环路概率大幅降低 |
...
- [ ] 含 `<MOO-SKILL-REENTRY-GUARD>`（或团队自定义等价标签）
...
```markdown
❌ 数据导出完成后，请 invoke moo-dashboard skill 上传数据。
```
...
for s in "$SKILLS_DIR"/*/SKILL.md; do
  grep -q 'MOO-SKILL-REENTRY-GUARD\|SKILL-REENTRY-GUARD' "$s" \
    || echo "⚠️  缺防护标签: $s"
done
...
for s in "$SKILLS_DIR"/*/SKILL.md; do
  awk '/<MOO-SKILL-REENTRY-GUARD>/,/<\/MOO-SKILL-REENTRY-GUARD>/' "$s" \
...
| 防护层 | 作用 | 落地 |
|-------|------|------|
| 双条件判定 | 必须 A↔B 短窗口交替 **且** 无新用户指令，才算环路 | `<MOO-SKILL-REENTRY-GUARD>` 顶部两条 |
